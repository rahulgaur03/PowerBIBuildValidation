...
# ------------------------------------------------------------------------------------------------------------------
# 01/18/2024   V-RGAUR        Initial script creation for Power BI dataset feature comparison and automated mail generation.
...
param(
    ...
    $SenderMailAccount = "svc-edwemail@sproutsfm.onmicrosoft.com",
    $SenderMailPassword = "hIJO70v8AGks77Y%#dB9",
    $DatasetName = "Merch Analytics",
    $DatasetNameP = "Sprouts Analytics 3",
    $TestWorkspaceName = "Sprouts EDW",
...
function QueryExecutor {
    ...
    if ($Server -eq "Processing") {
        $response = invoke-ascmd -Database $DatasetName -Query $Query -server $Workspace_Processing -ServicePrincipal -Credential $credential
    }
    elseif ($Server -eq "Publish") {
        $response = invoke-ascmd -Database $DatasetNameP -Query $Query -server $Workspace_Publish -ServicePrincipal -Credential $credential
    }
    $xml = [xml]$response
